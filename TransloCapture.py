...
# This program is used to identify reads of a fastq that are translocated sequences from a targeted sequencing experiment.
# This outputs a csv file of a matrix containing the rate of every possible crossover event as a percentage of the reads.
...
def argypargy():
    ...
    parser.add_argument("--primers", "-p", help="A .csv file containing the name and foward and reverse primer (reverse complement) sequences for each site to be analysed on each line.", required=True)
...
# Open/process the input and output files line by line to reduce memory demands
